...
export function renderHighlight(area, params) {
  ...
  // outer ring
  cr.setSourceRGBA(drawColor.r, drawColor.g, drawColor.b, drawColor.a);
  ...
  if (extraRingProgress > 0 && clickColor) {
    const expandScale = 1.0 + extraRingProgress * 0.8; // expand to 180%
    const expandOpacity = 1.0 - extraRingProgress; // fade out
    const clickLineWidth = Math.max(6, outerBorderWidth); // minimum 6px thick
    ...
    cr.setLineWidth(clickLineWidth);

    const expandHalf = (outerHalf - clickLineWidth / 2) * expandScale;
