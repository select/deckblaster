...
func (d *Deck) triggerAction(dev *streamdeck.Device, index uint8, hold bool) {
	for _, w := range d.Widgets {
		...
		if a.Exec != "" && a.Deck != "" {
			// Run exec synchronously BEFORE deck switch so state is set
		...
		if a.Exec != "" && a.Deck == "" {
			...
			go func() {
				executeCommand(a.Exec)
				// Give HA / the OS a moment to process, then force-refresh
				// this widget so the icon reflects the new state immediately.
				time.Sleep(600 * time.Millisecond)
				currentWidget.ResetUpdate()
				_ = currentWidget.Update()
			}()
		}
		if a.Device != "" {
			switch {
