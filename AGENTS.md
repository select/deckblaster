...
Screenshots in `docs/screenshots/` are generated by `scripts/make-screenshots.py`. The script stops the running service, launches deckmaster once per deck file, waits for widgets to render, captures via the `/screenshot` HTTP API, composites onto the Stream Deck photo template, and saves the result. The **GitHub screenshot** is generated with mock data (via `deckmaster/decks/github/make-screenshot.py`) to avoid exposing PII — this is called automatically by the main script.

```bash
uv run scripts/make-screenshots.py              # all decks (including github mock)
uv run scripts/make-screenshots.py main docker  # only specific decks
uv run scripts/make-screenshots.py github       # only github (mock data, no device needed)
```

The script restarts `deckblaster.service` when done. To add a new deck to the screenshot set, append an entry to the `DECKS_TO_SHOOT` list in the script.
